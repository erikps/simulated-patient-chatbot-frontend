...
export function parseResponse(action, sendMessageCallback, senderId) {
  let buttons = <></>;
  if (action?.buttons) {
    buttons = (
  ...
  let text = <></>;
  if (action?.text && action?.timestamp) {
    text = (
      <div className="d-flex flex-row align-items-start  bot-item justify-content-start">
        ...
        <div class="">
          <ReportButton senderId={senderId} timestamp={action.timestamp} />
        </div>
      </div>
    );
  }
